...
class ChatAdapter(Protocol):
    ...
    No service is connected in this project. An external implementation would
    need its own consent, credentials, privacy review, timeouts, and error policy.
    """

    def respond(self, message: object) -> ChatReply:
...
FAQ_RULES: tuple[Rule, ...] = (
    ...
    Rule("python", compile_pattern(r"\b(?:python|interpreter|pip)\b"), fixed_answer(
        ...
        "the extension does not install Python itself. This chatbot needs only the Python standard library.")),
    Rule("virtual_environment", compile_pattern(r"\b(?:venv|virtual environment|virtualenv|activate|activation)\b"), fixed_answer(
        "From the traditional_chatbot folder, run: py -3.12 -m venv .venv\n"
    ...
    Rule("tests", compile_pattern(r"\b(?:test|tests|testing|unittest|validate|validation)\b"), fixed_answer(
        "From the traditional_chatbot folder, run: .\\.venv\\Scripts\\python.exe -m unittest discover -s tests -v\n"
    ...
    Rule("chatbot", compile_pattern(r"\b(?:chatbot|algorithm|rules|traditional|llm|model)\b"), fixed_answer(
        ...
        "It does not train a model, call an AI service, or remember previous messages. "
...
class RuleBasedAdapter:
    ...
    def respond(self, message: object) -> ChatReply:
        # Validate before regex matching and bound input size for predictable work.
