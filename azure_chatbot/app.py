"""Local browser and JSON API for the hybrid Azure sentiment chatbot."""
...
from engine import ChatAdapter, InvalidMessage
...
def make_handler(adapter: ChatAdapter) -> type[BaseHTTPRequestHandler]:
    ...
    class ChatRequestHandler(BaseHTTPRequestHandler):
        server_version = "AzureHybridChatbot/1.0"
        sys_version = ""
        ...
        def send_bytes(self, status: int, body: bytes, content_type: str) -> None:
            ...
            try:
                ...
                pass  # The browser may have closed while Azure was responding.
        ...
        def do_POST(self) -> None:
            ...
            try:
                ...
                return
            if not isinstance(payload, dict) or "message" not in payload or set(payload) - {"message", "cloud_consent"}:
                self.send_json(400, {"error": "Send a JSON object with a message field and optional boolean cloud_consent."})
                return
            try:
                reply = adapter.respond(payload["message"], cloud_consent=payload.get("cloud_consent", False))
            except InvalidMessage as error:
                self.send_json(400, {"error": str(error)})
                return
            except Exception:
                self.send_json(500, {"error": "The server could not complete the request. No result is available. Check the project configuration and try a local command."})
                return
...
def create_server(port: int = 3979, adapter: ChatAdapter | None = None) -> ThreadingHTTPServer:
    """Always bind to the loopback interface; port zero supports isolated tests."""
    return ThreadingHTTPServer(("127.0.0.1", port), make_handler(adapter if adapter is not None else HybridAdapter()))


def main() -> int:
    parser = argparse.ArgumentParser(description="Run the hybrid Azure sentiment chatbot locally.")
    parser.add_argument("--port", type=int, default=3979, help="Local port, default 3979")
    ...
    try:
        ...
        print(f"Unable to start: {error}. Try a different port, such as --port 3980.", file=sys.stderr)
        return 1
    with server:
        print(f"Open http://127.0.0.1:{arguments.port} in your browser.", flush=True)
        print("Local rules plus Azure sentiment with per-request permission. Press Ctrl+C to stop.", flush=True)
        try:
