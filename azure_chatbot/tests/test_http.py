...
class HTTPTests(unittest.TestCase):
    ...
    def test_cloud_permission_missing_or_false_prevents_azure(self):
        for payload in ({"message": "sentiment: example"}, {"message": "sentiment: example", "cloud_consent": False}):
            with self.subTest(payload=payload):
                status, _, body = self.post(payload)
                self.assertEqual(status, 200)
                self.assertEqual(json.loads(body)["reply"]["intent"], "consent_required")

    def test_non_boolean_cloud_permission_is_rejected(self):
        for consent in ("true", 1, None):
            with self.subTest(consent=consent):
                status, _, body = self.post({"message": "sentiment: example", "cloud_consent": consent})
                self.assertEqual(status, 400)
                self.assertIn("true or false", json.loads(body)["error"])

    def test_browser_contains_unchecked_per_request_consent(self):
        _, _, page = self.request("GET", "/")
        self.assertIn(b'type="checkbox" id="cloud-consent"', page)
        self.assertNotIn(b'id="cloud-consent" checked', page)
        self.assertIn(b'Permission applies to one request', page)


class CloudHTTPTests(unittest.TestCase):
    def test_offline_sdk_fixture_travels_through_http_route(self):
        # This is a synthetic SDK fixture, not evidence of an Azure connection.
        from azure_service import AzureSentimentService
        from hybrid import HybridAdapter
        from test_azure import FakeClient
        client = FakeClient()
        server = create_server(0, HybridAdapter(AzureSentimentService(lambda: client)))
        thread = threading.Thread(target=server.serve_forever, daemon=True)
        thread.start()
        connection = http.client.HTTPConnection("127.0.0.1", server.server_address[1], timeout=5)
        try:
            connection.request("POST", "/api/messages", json.dumps({"message": "sentiment: example text", "cloud_consent": True}), {"Content-Type": "application/json"})
            response = connection.getresponse()
            payload = json.loads(response.read())
            self.assertEqual(response.status, 200)
            self.assertEqual(payload["reply"]["source"], "azure")
            self.assertEqual(payload["reply"]["analysis"]["sentiment"], "mixed")
            self.assertEqual(client.calls[0]["documents"], ["example text"])
        finally:
            connection.close()
            server.shutdown()
            server.server_close()
...
class AdapterBoundaryTests(unittest.TestCase):
    def test_server_can_use_an_injected_adapter(self):
        class FakeAdapter:
            def respond(self, message, *, cloud_consent=False):
